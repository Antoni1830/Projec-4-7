...
const response= await mongodb.getDatabase().db().collection('Games007').insertOne(user);
...
const response= await mongodb.getDatabase().db().collection('Games007').replaceOne({id: userId}, user);
...
const deleteUser = async (req, res) => {
    //#swagger.tags=['Games']
  const userId = new ObjectId(req.params.id);
  const response = await mongodb.getDatabase().db().collection('Games007').deleteOne({ _id: userId }, true);
